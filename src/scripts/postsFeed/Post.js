import {getUsers, getLikes, getCurrentUser, getPosts, sendLike, deleteLike, fetchLikes} from "../dataAccess.js";
...
const postInnerHtml = (post) => {
  ...
  return `
    ...
    <img class="postItem postLikeBtn" id="likeBtn--${post.id}" src="${likes.find(like => like.userId === currentUser.id && like.postId === post.id) ? "./images/favorite-star-yellow.svg" : "./images/favorite-star-blank.svg"}" width="20px" />`
}
  ...
  if (clickEvent.target.id.startsWith("likeBtn--")) {
    ...
    if (thisPostLikeObj) {
      deleteLike(thisPostLikeObj.id)
        .then(() => fetchLikes())
        .then(() => 
          document.querySelector(`#post--${postId}`).innerHTML = postInnerHtml(posts.find(post => post.id === postId))
        );
    } else {
      const likeToSend = {
